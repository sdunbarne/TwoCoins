...
Recall
the \defn{Total Variation distance} of \( \bbP^m \) from \(
    \bbQ^{m} \) is%
...
$\mu$ compared to $\nu$.
\index{Kullback-Liebler Divergence}.  The Kullback-Liebler Divergence
...
\link{http://www.math.unl.edu/~sdunbar1/ProbabilityTheory/Lessons/BernoulliTrials/LargeDeviations/largedeviations.html}{Large
  Deviations} in the section on Large Deviations.
...
\[
  m \ge \frac{\log(1/\alpha}{D_{KL}(\frac{1}{2} + \frac{\epsilon}{2}
    \parallel \frac{1}{2})}.
\]
As shown above  ${D_{KL}(\frac{1}{2} + \frac{\epsilon}{2}
    \parallel \frac{1}{2}) \le 2 \epsilon^{-2}$ so choosing $m$ at least
$2 \log(1/\alpha) \epsilon^{-2}$ suffices.
...
begining, $\alpha = 0.05$ and $\epsilon = 0.10$, the lower bound is 
\[
  (1 - 2\delta)^2 \epsilon^{-2} = (1- 2 \cdot 0.05)^2 \cdot
  (0.10)^{-2} = 81
  \]
  and the upper bound is
  \[
    2 \log(1/\alpha) \epsilon^{-2} = 2 \log(1/0.05) (0.10)^{-2} \approx 599.15.
  \]
...
The necessary bounds for an arbitrary algorithm are adapted from
lecture notes by Austin Eide.
